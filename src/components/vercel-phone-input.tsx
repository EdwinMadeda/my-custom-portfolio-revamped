...
> &
  Omit<RPNInput.Props<typeof RPNInput.default>, "onChange"> & {
    ...
    priorityCountries?: RPNInput.Country[];
  };

const VercelPhoneInput: React.ForwardRefExoticComponent<PhoneInputProps> =
  React.forwardRef<
    React.ComponentRef<typeof RPNInput.default>,
    PhoneInputProps
  >(({ className, onChange, value, priorityCountries = [], ...props }, ref) => {
    const [selectedCountry, setSelectedCountry] = React.useState(
      props.defaultCountry ?? "KE",
    );
    ...
    const getCountryList = React.useMemo(() => {
      return (countryList: CountryEntry[]): CountryEntry[] => {
        const countriesByCode = Object.fromEntries(
          countryList.map((entry) => [entry.value ?? "", entry]),
        );
        ...
        return [
          ...
          ...countryList.filter(
            (country) =>
              !priorityCountries.includes(country.value ?? selectedCountry),
          ),
        ];
      };
    }, [priorityCountries]);
    ...
    const CountrySelectOptimized = ({
      ...
      return (
        <CountrySelect
          options={getCountryList(countryList)}
          {...countrySelectProps}
        />
      );
    };
    ...
    React.useEffect(() => {
      if (form && selectedCountry) {
        form.setValue("phoneCountry", selectedCountry, {
    ...
    return (
      <RPNInput.default
        ...
        countrySelectComponent={CountrySelectOptimized}
...
VercelPhoneInput.displayName = "PhoneInput";
